...
/**
 * On-screen log viewer for the HUD. Same component as the Remote
 * version — captures console output and shows it in a bottom sheet
 * toggled by a small ⓘ button. Useful for debugging on Viture / any
 * device without chrome://inspect access.
 */
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <span style={{ color: '#00E5FF' }}>HUD log ({entries.length})</span>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button type="button" onClick={() => setOpen(false)} style={pillStyle}>
                ×
              </button>
            </div>
          </div>
          <div style={{ padding: '4px 8px' }}>
            {entries.length === 0 ? (
              <div style={{ opacity: 0.5, padding: '8px 4px' }}>
                Nessun log. Quando arrivano command/handoff events appariranno qui.
              </div>
            ) : (
              entries
                .slice()
