...
interface VoiceDto {
  ...
  /** True for voices created/cloned/generated by the user (not built-in). */
  isCustom: boolean;
...
function shapeVoice(v: ElevenVoice): VoiceDto {
  ...
  // Anything not premade is user-tuned content we want to highlight & always show.
  const cat = (v.category ?? '').toLowerCase();
  const isCustom = cat === 'cloned' || cat === 'generated' || cat === 'professional';
  return {
    id: v.voice_id,
    name: v.name,
...
/**
 * Fetch a voice's saved settings from ElevenLabs (the user's custom tuning).
 * Used by the admin UI to pre-fill the parameter sliders.
 */
adminVoicesRoutes.get('/:voiceId/settings', async (c) => {
  const cfg = c.get('config');
  const voiceId = c.req.param('voiceId');
  if (!voiceId) return c.json({ error: { code: 'BAD_REQUEST', message: 'missing voiceId' } }, 400);
  if (!cfg.ELEVENLABS_API_KEY) {
    return c.json({ error: { code: 'MISCONFIGURED', message: 'missing elevenlabs api key' } }, 500);
  }
  const res = await fetch(
    `https://api.elevenlabs.io/v1/voices/${encodeURIComponent(voiceId)}/settings`,
    {
      headers: { 'xi-api-key': cfg.ELEVENLABS_API_KEY, Accept: 'application/json' },
    },
  );
  ...
  const data = (await res.json()) as ElevenSettingsResp;
  return c.json({
